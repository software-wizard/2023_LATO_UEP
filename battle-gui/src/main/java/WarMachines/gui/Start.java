...
public class Start extends Application
{
    ...
    {
        return new NewHeroPrototype( List.of( new NecropolisFactory().create( false, 1, 5 ) ),
                List.of( new WarMachine.Builder().statistic(WarMachineStatistic.CATAPULT).amount(1).build() ));
    }

    private NewHeroPrototype createP1()
    {
        return new NewHeroPrototype( List.of( new NecropolisFactory().create( false, 1, 5 ) ),
                List.of( new WarMachine.Builder().statistic(WarMachineStatistic.BALLISTA).amount(1).build()) );
    }
