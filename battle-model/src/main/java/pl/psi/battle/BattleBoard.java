...
public class BattleBoard
{
    ...
    public BattleBoard( final List< ? extends MapObjectIf > mapObjectIfs1,
        final List< ? extends MapObjectIf > mapObjectIfs2 )
    {
        for( int x = 0; x < 15; x++ )
        {
            for( int y = 0; y < 10; y++ )
    ...
    {
        return getMapObject( point ).filter( Creature.class::isInstance )
    ...
    void move( final MapObjectIf mapObjectIf, final Point aPoint )
    {
        ...
        List< Point > theRightPath =
            path.findPath( gridConstruction( availablePointsToGo( mapObjectIf ) ), startingPoint, endPoint );
    ...
    boolean canMove( final MapObjectIf mapObjectIf, final Point aPoint )
    {
        ...
        return aPoint.distance( oldPosition ) <= mapObjectIf.getMoveRange();
    }

    Point getPosition( MapObjectIf mapObjectIf )
    {
        if( (map.inverse()
            .get( mapObjectIf )) != null )
        {
            return map.inverse()
                .get( mapObjectIf );
        }
