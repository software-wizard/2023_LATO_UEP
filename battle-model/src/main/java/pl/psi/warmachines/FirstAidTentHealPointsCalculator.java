...
public class FirstAidTentHealPointsCalculator implements FirstAidTentIf{
    ...
    public int calculateHealPoint(WarMachine aAttacker, MapObjectIf aAlly, int currentHP, int firstAidSkillLevel) throws Exception {

        int hp;
        if (aAttacker.getStats().equals(WarMachineStatistic.FIRST_AID_TENT)){
            hp = calculateHealPoints(firstAidSkillLevel, currentHP);
        } else{
